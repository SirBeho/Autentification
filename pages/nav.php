...
<div class="flex justify-between w-screen px-4  sx:px-8 py-4 relative">

    <!-- Sección del logotipo -->
    <div class="w-40">
        <!-- Mostrar logotipo "devchallenges" -->
        <img  src="../svg/devchallenges.svg" alt="logo" />
    </div>
    ...
    <div id="control-menu" class="flex items-center gap-2 cursor-pointer">
        ...
        <div class="h-8 w-8 overflow-hidden rounded-lg">
            ...
            <img id="imagePreviewmenu" class="w-full h-full object-cover" src="../pictures/<?php echo is_file("../pictures/photo_".$id) ? "photo_".$id : "usuario.jpg" ?>" alt="">
        </div>

        <!-- Mostrar nombre del usuario (oculto en pantallas pequeñas) -->
        <span class="hidden sx:block font-semibold text-xs leading-snug"><?php echo $name ?></span>

        <!-- Flecha hacia abajo (oculta en pantallas pequeñas) para indicar menú desplegable -->
        <div class="hidden sx:block w-4 rotate-180">
            <img src="../svg/arrow.svg" alt="logo" />
        </div>
    </div>

    <!-- Menú desplegable -->
    <div id="menu" class="border border-gray-BD rounded-xl p-2 w-36 bg-white text-xs absolute top-16 right-[5%] overflow-hidden h-0 opacity-0 transform duration-500 ease-in-out">
